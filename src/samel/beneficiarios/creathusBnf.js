    ...
    workbook.SheetNames.forEach((sheetName) => {
      ...
      if (data.length > 0) {
        ...
        const columnIndexes = {
          ...
          descEstipulanteIndex: columnNames.indexOf(normalizeColumnName("cns")),
        };
          ...
          return colunasOriginaisBnf.map((col) => {
            ...
            switch (col) {
              ...
              case "DescricaoEstipulante":
                return columnIndexes.descEstipulanteIndex !== -1
                  ? (row[columnIndexes.descEstipulanteIndex] || "").toString()
                  : "";
              case "NrOperadora":
...
const findAndProcessFile = (directory) => {
  fs.readdir(directory, (err, files) => {
    ...
    const targetFile = files.find(
      (file) =>
        file.includes("2037 - CREATHUS") && path.extname(file) === ".xlsx"
    );

    if (!targetFile) {
      console.error('Nenhum arquivo encontrado com "2037 - CREATHUS" no nome.');
      return;
    }
