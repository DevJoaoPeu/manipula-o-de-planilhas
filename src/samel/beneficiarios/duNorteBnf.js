    ...
    workbook.SheetNames.forEach((sheetName) => {
      ...
      if (data.length > 0) {
        ...
        const columnIndexes = {
          ...
          descSubEstipulanteIndex: columnNames.indexOf(
            normalizeColumnName("subestipulante")
          ),
        };
            ...
            switch (col) {
              ...
              case "Cidade":
                return columnIndexes.municipioIndex !== -1
                  ...
                  : "";
              case "DescricaoSubEstipulante":
                return columnIndexes.descSubEstipulanteIndex !== -1
                  ? row[columnIndexes.descSubEstipulanteIndex] || ""
                  : "";
              case "DescricaoSubFatura":
                return columnIndexes.contratanteIndex !== -1
                  ? row[columnIndexes.contratanteIndex] || ""
...
const findAndProcessFile = (directory) => {
  fs.readdir(directory, (err, files) => {
    ...
    const targetFile = files.find(
      (file) =>
        file.includes("1750 - DUNORTE") && path.extname(file) === ".xlsx"
    );

    if (!targetFile) {
      console.error('Nenhum arquivo encontrado com "1750 - DUNORTE" no nome.');
      return;
    }
