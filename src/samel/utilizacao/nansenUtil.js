...
import { normalizeColumnName, nameUtil } from "../util/utils.js";
...
const findAndProcessFile = (directory) => {
  fs.readdir(directory, (err, files) => {
    ...
    // Filtra os arquivos que contenham "NANSEN" no nome e tenham a extensão .xlsx
    const targetFile = files.find(
      (file) => file.includes("NANSEN") && path.extname(file) === ".xlsx"
    );

    if (!targetFile) {
      console.error('Nenhum arquivo encontrado com "NANSEN" no nome.');
      return;
    }
